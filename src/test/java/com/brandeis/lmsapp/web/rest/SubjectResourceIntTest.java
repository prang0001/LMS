...
public class SubjectResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllSubjectsByLibraryResourceIsEqualToSomething() throws Exception {
        // Initialize the database
        LibraryResource libraryResource = LibraryResourceResourceIntTest.createEntity(em);
        em.persist(libraryResource);
        em.flush();
        subject.addLibraryResource(libraryResource);
        subjectRepository.saveAndFlush(subject);
        Long libraryResourceId = libraryResource.getId();

        // Get all the subjectList where libraryResource equals to libraryResourceId
        defaultSubjectShouldBeFound("libraryResourceId.equals=" + libraryResourceId);

        // Get all the subjectList where libraryResource equals to libraryResourceId + 1
        defaultSubjectShouldNotBeFound("libraryResourceId.equals=" + (libraryResourceId + 1));
    }
